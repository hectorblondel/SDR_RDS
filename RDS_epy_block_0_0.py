...
class blk(gr.sync_block):  # other base classes are basic_block, decim_block, interp_block
    ...
    def __init__(self, t0=0, nb_ech_par_symb=8):  # only default arguments here
        ...
        gr.decim_block.__init__(
            ...
            in_sig=[np.complex64],
            out_sig=[np.complex64,np.complex64,np.complex64,np.complex64,np.complex64],
    ...
    def work(self, input_items, output_items):
        ...
        imax = 0
        vmax = np.mean([x for x in output_items[0][:] if np.real(x) > 0]) * np.mean([x for x in output_items[0][:] if np.real(x) <0 ])
        for i in range(1,4):
            v = np.mean([x for x in output_items[i][:] if np.real(x) > 0]) * np.mean([x for x in output_items[i][:] if np.real(x) <0 ])
            if v > vmax :
                i = imax
                vmax = v
        
        output_items[4][:] = output_items[i][:]
        
        
        
        
        #code_differentiel = np.array([ np.sign(np.real(x)) for x in output_items[4]])
